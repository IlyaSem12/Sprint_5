...
class TestLogoutUser:
    def test_main_page_success_logout_user_is_displayed_login_and_registration_buttton(self,browser,login_user):
        ...
        assert webdrivwer.until(expected_conditions.visibility_of_element_located(LOGIN_AND_REGISTRATION_BUTTON)), 'Отсутвует кнопка «Вход и регистрация»'
    ...
    def test_main_page_success_logout_user_is_not_displayed_user_avatar(self, browser, login_user):
        ...
        webdrivwer =  WebDriverWait(browser, 5)
        webdrivwer.until(expected_conditions.element_to_be_clickable(EXIT_BUTTON)).click()
        assert webdrivwer.until(expected_conditions.invisibility_of_element(AVATAR_BUTTON)), "Аватар пользователя должен отсутствовать" 
    ...
    def test_main_page_logout_user_is_not_displayed_user_name(self,browser,login_user):
        ...
        webdrivwer =  WebDriverWait(browser, 5)
        webdrivwer .until(expected_conditions.element_to_be_clickable(EXIT_BUTTON)).click()
        assert WebDriverWait(browser, 3).until(expected_conditions.invisibility_of_element(USERNAME_HEADER)), "Имя пользователя должено отсутствовать"